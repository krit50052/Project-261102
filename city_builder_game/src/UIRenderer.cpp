...
void UIRenderer::renderNodeInfo(sf::RenderWindow& window, MapNode* node) {
    ...
    switch (node->getOwner()) {
        case Owner::Player:
            ownerStr += "Player";
            ownerColor = sf::Color(60, 120, 216);
            break;
        case Owner::Enemy:
            ownerStr += "Enemy";
            ownerColor = sf::Color(220, 60, 60);
            break;
        case Owner::Neutral:
            ownerStr += "Neutral";
            ownerColor = sf::Color(160, 160, 170);
            break;
    }
...
bool UIRenderer::renderEndTurnButton(sf::RenderWindow& window, sf::Vector2i mousePos, bool mouseClicked) {
    ...
    if (isHovered) {
        button.setFillColor(sf::Color(80, 180, 80)); // Bright green when hovered
    } else {
        button.setFillColor(sf::Color(60, 150, 60)); // Normal green
    }
...
BuildingType UIRenderer::renderBuildMenu(sf::RenderWindow& window, MapNode* node, const GameResources& resources, sf::Vector2i mousePos, bool mouseClicked) {
    ...
    // Building options
    BuildingType buildings[] = {BuildingType::Farm, BuildingType::Mine, BuildingType::Lumbermill, BuildingType::Barracks};
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (!canAfford) {
            button.setFillColor(sf::Color(60, 40, 40)); // Dark red if can't afford
        } else if (isHovered) {
            button.setFillColor(sf::Color(80, 100, 140)); // Highlight
        } else {
            button.setFillColor(sf::Color(50, 60, 80)); // Normal
        }
...
bool UIRenderer::renderCaptureButton(sf::RenderWindow& window, MapNode* node, const GameResources& resources, sf::Vector2i mousePos, bool mouseClicked) {
    ...
    // We'll implement this check in Game.cpp
    
    // Capture panel (right side)
    ...
    if (!canAfford) {
        ...
        button.setFillColor(sf::Color(100, 140, 220));
    } else {
        button.setFillColor(sf::Color(60, 100, 180));
    }
...
UnitType UIRenderer::renderRecruitMenu(sf::RenderWindow& window, MapNode* node, const GameResources& resources, sf::Vector2i mousePos, bool mouseClicked) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        statsText.setString("ATK:" + std::to_string(stats.attack) + " DEF:" + std::to_string(stats.defense) + " Counters:" + getUnitName(stats.counters));
...
int UIRenderer::renderMoveMenu(sf::RenderWindow& window, MapNode* currentNode, const std::vector<int>& destinations, const std::vector<std::string>& destinationNames, sf::Vector2i mousePos, bool mouseClicked) {
